...
def predictSubtype(df_decomp,no_table_sort=False, use_default_clfs=False):
    ...
    dict_cluster_numbers={"SMK":0, "UVL":1, "APB":2, "POL":3, "MRD":4,"HRD":5,
                        "GNS":6,"AGE":7,"UND":8}
    dict_cluster_points={"SMK":2, "UVL":2, "APB":2, "POL":2, "MRD":2,"HRD":0,"GNS":0,"AGE":0}
    dict_cluster_points2={"SMK":2, "UVL":2, "APB":2, "POL":2, "MRD":2,"HRD":1,"GNS":1,"AGE":0}
    ...
    tgs_high=[]
    for idx in df_preds.index:
        if df_preds.loc[idx,"Cons"] in ["SMK","APB","UVL","MRD","POL"] :
            tgs.append("high")
            tgs_high.append(1)
        elif df_preds.loc[idx,"Cons"] in ["AGE","GNS","HRD"]:
            tgs.append("low")
            tgs_high.append(0)
        else:
            tgs.append("und")
            tgs_high.append(np.nan)
    df_preds["TGS"]=tgs
    df_preds["TGS_high"]=tgs_high
    
    df_point=df_preds.iloc[:,0:4].replace( dict_cluster_points )
    df_point2=df_preds.iloc[:,0:4].replace( dict_cluster_points2 )
    df_preds["TGS_score1"]=np.sum(df_point,axis=1)
    df_preds["TGS_score2"]=np.sum(df_point2,axis=1)
    df_preds["TGS_high1"]=(df_preds["TGS_score1"]>=6).astype(int)
    df_preds["TGS_high2"]=(df_preds["TGS_score2"]>=6).astype(int)
    ...
    if no_table_sort:
        ...
        df_numbers_sort=df_numbers.sort_values( ["Cons","TopCounts","KNN","SVC","RF","LR"],
...
def visualizePrediction(df_decomp,df_preds,out_f,figure_size="5,5",marker_size=None,use_default_umap=False):
    ...
    colors=[ dict_cluster_colors[cluster] for cluster in df_preds_order["Cons"]]
    ## set marker size
    markeredgewidth = 1
